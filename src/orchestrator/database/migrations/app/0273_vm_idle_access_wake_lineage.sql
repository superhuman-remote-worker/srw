...
-- A later explicit Resume is separate from the immutable access-only Ready
-- receipt. It may join only its still-open, exact successor operation.
ALTER TABLE public.vm_idle_operations ADD COLUMN post_ready_resume_requested boolean NOT NULL DEFAULT false;
