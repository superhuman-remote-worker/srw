    ...
    return JobControlOperations(
        SimpleNamespace(
            ...
            completion_control=SimpleNamespace(
                dispatch_guard_kwargs=lambda: {},
                active_claim=lambda _job: False,
            ),
...
def test_inherited_child_cannot_select_parent_vm_for_terminal_cleanup():
    child = {
        "parent_job_id": "bbbbbbbb-cccc-4ddd-8eee-ffffffffffff",
...
async def test_duplicate_non_vm_or_inherited_stateless_cancel_settles_marker(inherited):
    ...
    if inherited:
        context.update({"vm": VM, "inherits_parent_workspace": True})
    job = {
        "id": JOB_ID,
        "status": "cancelled",
        "execution_lane": "stateless",
        "parent_job_id": "bbbbbbbb-cccc-4ddd-8eee-ffffffffffff" if inherited else None,
        "context": context,
    }
    ...
    store = SimpleNamespace(
        cancel_stateless_job=AsyncMock(return_value=(False, False)),
        ...
        finalize_cancelled_stateless_job=AsyncMock(return_value=True),
        complete_stateless_cancel_cleanup=AsyncMock(return_value=True),
    )
    archive = AsyncMock()
    operation = controls(store=store, archive=archive)
    operation.cascade_cancel_to_children = AsyncMock(return_value=True)

    assert await operation.cancel(JOB_ID, job=job.copy()) == {"status": "cancelled"}
    operation.cascade_cancel_to_children.assert_awaited_once_with(JOB_ID)
    archive.assert_awaited_once_with(JOB_ID)
    store.complete_stateless_cancel_cleanup.assert_awaited_once_with(JOB_ID)


@pytest.mark.asyncio
async def test_duplicate_stateless_cancel_with_empty_context_keeps_old_settle_path():
    job = {
        "id": JOB_ID,
        "status": "cancelled",
        "execution_lane": "stateless",
    ...
    assert await operation.cancel(JOB_ID, job=job.copy()) == {"status": "cancelled"}
    operation.cascade_cancel_to_children.assert_awaited_once_with(JOB_ID)
    operation.wait_for_stateless_cancel_settle.assert_awaited_once_with(JOB_ID)


@pytest.mark.asyncio
async def test_duplicate_owned_vm_deleting_still_returns_fast_pending():
    job = {
        "id": JOB_ID,
        "status": "cancelled",
        "execution_lane": "stateless",
    ...
    operation = controls(store=store)
    operation.cascade_cancel_to_children = AsyncMock(
        side_effect=AssertionError("owned VM request entered slow cleanup")
    )

    assert await operation.cancel(JOB_ID, job=job.copy()) == {
        "status": "cancelled",
        "cleanup_pending": True,
    }
    operation.cascade_cancel_to_children.assert_not_awaited()
...
async def test_reconcile_ordinary_terminal_vm_uses_single_owner_and_strict_archive():
    ...
    store.complete_terminal_vm_cleanup_marker.assert_awaited_with(
        JOB_ID,
        expected_generation=VM["provision_generation"],
    )
...
async def test_hung_descendant_is_bounded_and_does_not_hide_next_candidate(monkeypatch):
    ...
    monkeypatch.setattr(
        module, "TERMINAL_VM_CLEANUP_TIMEOUT_SECONDS", 0.02, raising=False
    )
    ...
    assert (
        await asyncio.wait_for(
            operation.reconcile_terminal_vm_cleanups(limit=2),
            0.3,
        )
        == 1
    )
...
async def test_slow_snapshot_does_not_block_another_terminal_vm_candidate():
    ...
    store = SimpleNamespace(
        ...
        get_job=AsyncMock(side_effect=lambda job_id: rows[job_id]),
        stateless_cancel_cleanup_lock=lock,
        complete_terminal_vm_cleanup_marker=AsyncMock(
...
@pytest.mark.asyncio
async def test_terminal_vm_archive_keeps_reserved_storage_and_attests_charge(
    monkeypatch,
):
