...
**Published checkpoint (2026-09-20):** Reviewed foundations below are on develop
through `11a33debf`. This includes dormant policy installation/draining and its
tests (`83ac143f0`), which supersedes the earlier policy-lifecycle TODOs below.
Runtime integration, full launcher-cost contract, exact Node/effect binding,
physical reservation release and occupancy UX remain unfinished. All resource
admission modes default off; no production resource budgets were selected.
See the [delegation handoff](2026-09-20-vm-delegation-handoff.md).

## Implementation history and detailed plan

**Status:** implementation under the owner-approved remaining roadmap. D1 resource/placement primitives (`c62177172`), D2 sanitized collector (`5b3219e07`) and immutable inventory store/migration 0263 (`24feabea8`) passed independent review. Authenticated ingestion, default-off observer runtime and shared Helm policy (`94b13145a`) passed independent review, 133 inventory/Helm checks and 23 startup/runtime compatibility tests. The endpoint inventory now recognizes the existing fixed-purpose publisher authentication gate (`c9a01ab52`) without changing permissions or configuration. Direct read-only collection on dedicated k3d returned complete eight-kind inventory. A temporary observer-only rollout at frozen `5a9935c0b` also proved signed HTTP 200 publication and unsigned HTTP 400 rejection; observer configuration/RBAC were removed afterward and the disabled endpoint returned HTTP 503. No admission or enforcement is claimed. D3 exact occupancy (`bb1b26daa`, 24 tests), persistent fairness nomination (`7596fd415`, 12 tests), immutable reservation schema/evidence retention (`0e1ee5a88`, 24 PostgreSQL/head tests), and the unconnected atomic admission store (`e182b1e37`, 32 PostgreSQL/placement checks) passed independent review. Bounded invalid-head maintenance (`9b2c5fe00`, corrected by `be992cc33`) also passed review with 25 PostgreSQL maintenance checks: transient blockers park without losing age/protection, terminal no-effect requests can cancel, and malformed lineage quarantines only its own reservation-free waiter so unrelated owners can advance. These stores remain unconnected and all observer, shadow and enforcement modes remain off by default. Trusted waiter projection, runtime maintenance wiring, reservation binding/release, lifecycle integration, accounting visibility and enforcement remain incomplete. Production policy values and live acceptance remain rollout prerequisites.

The first trusted-template prerequisite is reviewed (`5bf49d0e7`, corrected by `d862054d8`): a pure bounded parser reads the original VM YAML without credential rendering and projects only supported CPU/memory, placement and ordinary root-storage semantics. Unsupported scheduling, devices, dynamic placement and hidden PV binding/population constraints are refused. All 59 parser/placement checks and three independent storage-constraint repros passed. The subsequent probe correction (`ba47429f7`) rejects unprojected custom probes, including exec probes with additional launcher overhead; 70 checks and the two original independent cases passed.

The complete immutable policy snapshot (`6fb27bbb8`) and private configuration-v2 bridge (`d2395a670`) also passed independent review. The snapshot freezes the full canonical operator document, reuses resource-value parsing with the reservation store, and rejects inconsistent or incorrectly typed derived values. A keyword-only internal opt-in adds the supported static profile, complete policy identity and recomputable cost estimate to frozen configuration. Existing callers retain v1; a literal pre-change golden fixture verifies its bytes/digest, and signed HTTP clients cannot supply the opt-in. Verification: 97 policy/reservation checks including actual PostgreSQL, 147 configuration/template/policy checks, and 17 independent v2 checks. This is an unconnected estimate, not a proven launcher request or resource grant. Trusted owner/project/priority projection, policy lifecycle, measured host-cost mapping, final rendered-manifest validation, runtime integration and reservation binding/release remain required. Observer-only snapshot validation does not relax the existing shadow/enforcement refusal.

Pure final-manifest validation (`0fd9ea316`) is now reviewed as well. Separate VM and root-DV validators compare actual `CreationActuator.body` output with the digest-bound original template, frozen request/configuration and exact effect intent. Registry, golden, prepared and retained-root cases are covered; an existing retained root cannot authorize a new DV. Hostname binding preserves every original affinity branch, including impossible empty branches. All 241 related checks and seven independent adversarial checks passed. These helpers grant no authority and are not yet called by the effect path; current Node UID/topology, full launcher contract and reservation/carrier integration remain required.

A disposable local-k3d measurement on KubeVirt 1.6.6/amd64 now validates the distinction between compute-container and whole-Pod cost. With 2 vCPUs and 512Mi guest RAM, compute requested 200m/788Mi, but the default restartable `guest-console-log` init container added 5m/35M (decimal). The shared normalizer measured **205m, 861277888 bytes and one KVM device**. The earlier illustrative 0m fixed CPU / 260Mi fixed RAM policy only covered compute and underestimates this Pod. An explicit test policy with 5m fixed CPU and `260Mi + 35000000` fixed RAM matched this fixture; it is not a production default. The future launcher contract must bind console-log enablement, support-container overrides and every sidecar/init request as well as the existing architecture/ratio/defaulting inputs. A normalized captured fixture and regression preserve this finding. The disposable namespace was removed with a UID precondition and all four original application PVC/PV identities were preserved. This does not advance job-readiness, Longhorn or recovery-fault acceptance. Raw evidence: `launcher-cost-d2395a670.json`, SHA-256 `b28e8b3da3b6f850d84240c8bf40f6063eda59ad8ed5c6cf0020a514033f3846` in the local validation evidence directory.

Trusted waiter insertion is now reviewed (`991affa74`): an internal dependency in retry admission derives the waiter from the locked Job, frozen v2 request and exact installed policy. It captures owner/project/priority once, refuses backfill for older retries without a waiter, and recomputes demand/placement before both new admission and held-reservation replay. Refusal rolls back retry insertion and Resume. All 174 combined checks and three independent PostgreSQL/projection probes passed. The production dependency remains absent. Policy installation/draining, the full measured launcher contract, final Node/effect binding and reservation release remain before runtime integration or enforcement.

**Snapshot inspected:** initial implementation-seam audit at worktree HEAD `91f9971e14ade45d154bf8bff946dfaed3700b85` on 2026-09-20; reviewed follow-up checkpoints are recorded above. Refresh line numbers before further implementation.
