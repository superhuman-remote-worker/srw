...
The first adapter records only exact human-addressed VM Job routes under the existing pinned-agent or stateless-token publication transaction. Officer-only routes, shared/ambiguous owners and unknown runtime identity remain ineligible. Execution Resume and the reviewed stateless routed-reply exit close the episode only in their successful guarded statement, even after tracking is disabled; pure reprovisioning preserves it. Completion/other decision writers, officer escalation, semantic thread producers, terminal/alternative Resume writers, legacy in-process approval and runtime rebinding still need adapters before stage 1 is complete. `WORKSPACE_IDLE_RELEASE_ENABLED` is currently an internal default-off metadata gate, with no Helm enablement or lifecycle loop.
